...
module.exports = {
    name: "play",
    description: "Phát các bài hát yêu thích của bạn",
    usage: "[query]",
    permissions: {
    ...
    run: async (client, message, args, { GuildDB }) => {
        if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ | ** Bạn phải ở trong kênh thoại để phát nội dung nào đó! **");
       ...
       return client.sendTime(message.channel,"❌ | ** Nút Lavalink không được kết nối **");
        }
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`Danh sách phát đã được thêm vào hàng đợi`, message.author.displayAvatarURL());
                    ...
                    return client.sendTime(message.channel, "** Không tìm thấy kết quả phù hợp nào cho - **" + SearchString);
                }
            } else {
                let Searched = await player.search(SearchString, message.author);
                if (!player) return client.sendTime(message.channel, "❌ | ** Hiện không có gì đang phát ... **");

                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "** Không tìm thấy kết quả phù hợp nào cho - **" + SearchString);
    ...
    SlashCommand: {
        ...
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            let CheckNode = client.Manager.nodes.get(client.config.Lavalink.id);
            if (!CheckNode || !CheckNode.connected) {
              return client.sendTime(interaction,"❌ | ** Nút Lavalink không được kết nối **");
            }
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    case "LOAD_FAILED":
                        ...
                        return client.sendError(interaction, `❌ |** Đã xảy ra lỗi khi tìm kiếm **`);

                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                        return client.sendTime(interaction, "❌ |**Không tim thây kêt quả.**");
                    case "TRACK_LOADED":
                ...
                try {
                    ...
                    if (res.loadType === "LOAD_FAILED") {
                        if (!player.queue.current) player.destroy();
                        return client.sendError(interaction, `:x: |** Đã xảy ra lỗi khi tìm kiếm **`);
                    }
                } catch (err) {
                    return client.sendError(interaction, `Đã xảy ra lỗi khi tìm kiếm: ${err.message}`);
                }
                switch (res.loadType) {
                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
